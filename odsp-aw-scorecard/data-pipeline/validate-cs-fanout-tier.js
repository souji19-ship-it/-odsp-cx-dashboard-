...
const OUT = path.join(__dirname, process.env.CS_VALIDATE_OUT || 'cs-validate-tier'); fs.mkdirSync(OUT, { recursive: true });
const CLUSTERS = [
 ...
 'fdislandsae.uaenorth','fdislandssg.southeastasia',
 'fdislandsuk.uksouth','fdislandsza.southafricanorth'];

const START_WEEK = process.env.CS_START_WEEK || '2026-07-12';
const END_WEEK = process.env.CS_END_WEEK || '';
const SCOPE = process.env.CS_SCOPE || 'odsp';
const KQL = `
let StartWeek = datetime(${START_WEEK});
let EndWeek = ${END_WEEK ? `datetime(${END_WEEK})` : 'now()'};
...
| where eventName in (${SCOPE === 'allup'
    ? "'AgenticLoopTurnLatency','AgenticLoopToolCallLatency','KnowledgeSourceLatency','AgenticLoopLlmCallLatency'"
    : "'AgenticLoopToolCallLatency','KnowledgeSourceLatency'"})
${SCOPE === 'allup' ? '' : `| where (eventName == 'AgenticLoopToolCallLatency' and (customDimensions contains 'shared_sharepointonline' or customDimensions contains 'shared_onedriveforbusiness'))
...
${SCOPE === 'allup' ? '' : '| where isTool or isKnow'}
...
| summarize Users = make_set(tostring(principalObjectId), 1048576),
            ...
            Tasks = ${SCOPE === 'allup'
              ? 'dcountif(tostring(cd.ConversationId), IsTurn)'
              : 'dcount(tostring(cd.ConversationId))'},
