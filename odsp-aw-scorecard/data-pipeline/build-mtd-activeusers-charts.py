...
# ODSP share of agent all-up active users (%), per week. Validated vs dashboard donuts
# (Cowork Aug 9-15 = 48%, Studio = 31%).
COWORK_SHARE = [round(o/a*100, 1) for o, a in zip(COWORK_ODSP, COWORK_ALLUP)]
STUDIO_SHARE = [round(o/a*100, 1) for o, a in zip(STUDIO_ODSP, STUDIO_ALLUP)]
...
def endlab(vals, ymax, cls, unit="count"):
    y = ymap(vals[-1], ymax)
    return f'<text class="el {cls}" x="{X1+6}" y="{y+3:.1f}">{fmt(vals[-1], unit)}</text>'

def chart(cowork, studio, unit="count"):
    ymax = nice_max(max(max(cowork), max(studio)))
    grid = ""
    for k in range(5):
        gv = ymax*k/4; gy = ymap(gv, ymax)
        grid += f'<line class="g" x1="{X0}" y1="{gy:.1f}" x2="{X1}" y2="{gy:.1f}"/>'
        grid += f'<text class="gt" x="{X0-6}" y="{gy+3:.1f}" text-anchor="end">{fmt(gv, unit)}</text>'
    ...
    body = (series_svg(cowork, ymax, "cw") + series_svg(studio, ymax, "cs")
            + endlab(cowork, ymax, "cw", unit) + endlab(studio, ymax, "cs", unit))
    axes = (f'<line class="ax" x1="{X0}" y1="{YTOP}" x2="{X0}" y2="{YB}"/>'
            f'<line class="ax" x1="{X0}" y1="{YB}" x2="{X1}" y2="{YB}"/>')
    return (f'<svg viewBox="0 0 {VBW} {VBH}" role="img" aria-label="Weekly trend">'
...
def panel(kind):
    ...
    if kind == "odsp":
        ...
        cw, cs = COWORK_SHARE, STUDIO_SHARE
        unit  = "pct"
        title = "ODSP share of M365 active users"
        sub   = "ODSP-active &divide; agent all-up &middot; weekly &middot; Jul 5 &ndash; Aug 15"
    else:
    ...
    def chip(lab, vals, cls):
        if unit == "pct":
            d = vals[-1] - vals[0]; dirn = "up" if d >= 0 else "down"
            return f'<span class="tp-chip {dirn}"><b>{lab}</b> {d:+.1f} pp Jul&rarr;Aug</span>'
...
with open(out, "w", encoding="utf-8") as fh:
    ...
    fh.write(panel("odsp") + "\n\n")
    fh.write("<!-- SHARE panel (M365 share section) -->\n")
    fh.write(panel("share") + "\n")
print("WROTE", out)
print("\n===ODSP===\n" + panel("odsp"))
print("\n===SHARE===\n" + panel("share"))
