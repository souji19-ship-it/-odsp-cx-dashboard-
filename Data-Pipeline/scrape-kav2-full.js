...
async function main() {
  ...
  try {
      ...
      if (subTabs === null) {
        ...
        for (const subTab of tabsToScrape) {
          ...
          try {
            ...
            // A renamed/removed tab must not abort the whole run — log and continue.
            console.log(`  ⚠️  Skipping "${subTab}" — ${subErr.message}`);
            allResults[topTabName][subTab] = { error: subErr.message };
          }
        }
      }
    }
    ...
    const summaryDir = path.join(__dirname, '..', 'Data-Public', 'spark');
    fs.mkdirSync(summaryDir, { recursive: true });
    const summaryFile = path.join(summaryDir, `kav2-full-summary-${timestamp}.json`);
