...
// One data root: IDEAS data lands in repo-root data/ideas/ (Copilot in SharePoint
// + other M365 Copilot surfaces). data-store reads ODSP_DATA_DIR at load, so set
// it BEFORE requiring the store.
process.env.ODSP_DATA_DIR = process.env.ODSP_DATA_DIR || path.join(__dirname, '..', 'Data-Public', 'ideas');

const ideas = require('./lib/ideas-client');
const store = require('./lib/data-store');

const MIN_SCRAPE_AGE_HOURS = parseInt(process.env.MIN_SCRAPE_AGE_HOURS || '6');
const TAB_PATH = 'm365-copilot';
