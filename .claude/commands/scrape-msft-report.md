...
3. Run all scrapers in parallel:
   ...
   - `--force` / `--fresh`     — bypass every scraper's freshness guard
     (`MIN_SCRAPE_AGE_HOURS=0` for all children). Use this when you specifically
     want today's numbers and don't care about the per-scraper "scraped <6h ago"
     skip.
...
| `ocv`        | `scrape-ocv.js`                   | OCV thumbs-down rate trends (daily/R7/R28), reliability, category breakdown (SPARK + AI Intranet) | Customer Voice tab |
...
- One-time SSO: first run will require signing in with @microsoft.com to Nezha,
  Power BI (`msit.powerbi.com`), SharePoint, and OCV (`ocv.microsoft.com`).
...
- `ocv` has the same freshness guard (`MIN_SCRAPE_AGE_HOURS=0 node scrape-ocv.js`
  to force). Adjust window with `OCV_DAYS=180` (default 90).
